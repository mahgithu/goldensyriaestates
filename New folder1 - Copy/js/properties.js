...
function initPropertiesPage() {
  ...
  if (typeF) typeF.innerHTML = PROPERTY_TYPES.map(t => `<div class="filter-option" data-val="${t.id}" data-group="type" onclick="toggleFilter(this)"><div class="filter-checkbox"><svg width="10" height="10" viewBox="0 0 12 12"><polyline points="2,6 5,9 10,3" fill="none" stroke="#0A1628" stroke-width="2"/></svg></div><span>${t.icon} ${t.name}</span></div>`).join('');
  if (govF) govF.innerHTML = GOVERNORATES.map(g => `<div class="filter-option" data-val="${g.id}" data-group="gov" onclick="toggleFilter(this)"><div class="filter-checkbox"><svg width="10" height="10" viewBox="0 0 12 12"><polyline points="2,6 5,9 10,3" fill="none" stroke="#0A1628" stroke-width="2"/></svg></div><span>${g.icon} ${g.name}</span></div>`).join('');

  // Apply URL params
  if (params.get('gov')) { activeFilters.gov = [params.get('gov')]; if (govSel) govSel.value = params.get('gov'); }
  if (params.get('type')) { activeFilters.type = [params.get('type')]; if (typeSel) typeSel.value = params.get('type'); }
  if (params.get('listing')) { activeFilters.listing = [params.get('listing')]; }
...
function toggleFilter(el) {
  ...
  if (idx >= 0) activeFilters[group].splice(idx, 1); else activeFilters[group].push(val);
  applyFilters();
}
...
function applyFilters() {
  ...
  if (!props.length) { grid.innerHTML = ''; if (empty) empty.style.display = 'block'; return; }
